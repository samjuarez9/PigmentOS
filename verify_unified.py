...
def test_unified_filter():
    print("🧪 Testing Unified 30-Day DTE Filter...")
    ...
    # 2. Long DTE (should be REMOVED from both)
    ...
    # Logic from run.py:
    # 1. Filter trades
    # 2. Update CACHE["whales"]["data"]
    # 3. Update CACHE["whales_30dte"]["data"]
    ...
    run.CACHE["whales"]["data"] = filtered_whales
    ...
    # Assertions
    if len(run.CACHE["whales"]["data"]) == 1:
        trade = run.CACHE["whales"]["data"][0]
        if trade["ticker"] == "SHORT_DTE":
            print("✅ Main Cache: Contains ONLY short DTE trade (PASSED)")
        else:
            print(f"❌ Main Cache: Wrong trade {trade['ticker']}")
    else:
        print(f"❌ Main Cache: Wrong size {len(run.CACHE['whales']['data'])}")
